...
class DefaultDataset(Dataset):
    ...
    def get_data_list(self):
        logger = get_root_logger()
        logger.info(f"DEBUG: Getting data list for split='{self.split}', data_root='{self.data_root}'")
        logger.info(f"DEBUG: Current working directory: {os.getcwd()}")
        logger.info(f"DEBUG: Data root exists: {os.path.exists(self.data_root)}")
        ...
        for split in split_list:
            ...
            logger.info(f"DEBUG: Processing split '{split}', path: '{split_path}'")
            logger.info(f"DEBUG: Split path exists: {os.path.exists(split_path)}")

            if os.path.isfile(os.path.join(self.data_root, split)):
                logger.info(f"DEBUG: Found file split, loading from file")
                with open(os.path.join(self.data_root, split)) as f:
                    data_list += [
                        os.path.join(self.data_root, data) for data in json.load(f)
                    ]
            else:
                logger.info(f"DEBUG: Found directory split, using glob")
                glob_pattern = os.path.join(self.data_root, split, "*")
                logger.info(f"DEBUG: Glob pattern: '{glob_pattern}'")
                glob_results = glob.glob(glob_pattern)
                logger.info(f"DEBUG: Glob returned {len(glob_results)} results")
                if len(glob_results) > 0:
                    logger.info(f"DEBUG: First few glob results: {glob_results[:3]}")
                else:
                    logger.warning(f"DEBUG: No glob results found!")
                    # Check if directory exists and list its contents
                    if os.path.exists(split_path):
                        logger.info(f"DEBUG: Directory contents: {os.listdir(split_path)[:10]}")
                data_list += glob_results

        logger.info(f"DEBUG: Final data list length: {len(data_list)}")
        return data_list
